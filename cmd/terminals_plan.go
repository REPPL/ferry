...
func planTerminals(ctx *cmdContext, home string, lastApplied *dotfile.Store) ([]planItem, []string, error) {
	...
	for _, ti := range tItems {
		planned[ti.Key] = true
		// Terminal config is not secret-rendered (secretRouted=false); its risk
		// comes from the three-way state alone (a first-touch adoption over an
		// existing file, or an overwrite of local edits, or a conflict).
		state, risky, reason, cerr := classifyItem(ti.Target, ti.Content, false, lastApplied)
		if cerr != nil {
