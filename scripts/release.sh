...
#                                        the real .work/NEXT.md is never touched)
#
...
usage: release.sh vX.Y.Z [--dry-run] [--yes]
  ...
  --dry-run   run gates 1-5, print what tag/push and the NEXT.md reset would do;
              create no tag or push and never touch .work/NEXT.md (the rehearsal
...
# render_next_md <src> <version> — print a reset .work/NEXT.md to stdout.
#
...
main() {
  ...
  # -------------------------------------------------------------------------
  # Step 7 — post-tag artefact reset of .work/NEXT.md.
  # -------------------------------------------------------------------------
  local next="$ROOT/.work/NEXT.md"
  if [ ! -f "$next" ]; then
    echo "release: no .work/NEXT.md — skipping the NEXT.md reset."
    return 0
  fi

  if [ "$DRY_RUN" -eq 1 ]; then
    echo "release: [dry-run] WOULD archive .work/NEXT.md to .work.local/history/NEXT-$VERSION-<timestamp>.md"
    echo "release: [dry-run] WOULD reset .work/NEXT.md; diff (current -> regenerated) below:"
    local tmp
    tmp="$(mktemp)"
  ...
  render_next_md "$next" "$VERSION" > "$tmp"
  mv "$tmp" "$next"
  echo "release: reset .work/NEXT.md (carry region preserved)."
}
